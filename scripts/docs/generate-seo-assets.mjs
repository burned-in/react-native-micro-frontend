...
const origin = normalizeOrigin(process.env.VITE_PUBLIC_SITE_ORIGIN || "https://burned-in.github.io");
const base = normalizeBase(process.env.GITHUB_PAGES_BASE || process.env.SITE_BASE || "/");
...
const manifest = {
  ...
  description: "Native-safe React Native micro frontend documentation with Hot Updater, host state, and native contract workflows.",
  start_url: withBase("/"),
  scope: withBase("/"),
  display: "standalone",
...
function createAlternates(group) {
  ...
  return [
    { lang: "x-default", href: absoluteUrl(group[0]) },
    ...alternates,
  ];
}
...
function withBase(route) {
  ...
  const path = base ? `${base}${normalizedRoute === "/" ? "/" : normalizedRoute}` : normalizedRoute;
