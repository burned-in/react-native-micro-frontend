...
const copy = {
  en: {
    suffix: brand,
    overview: "Native-safe React Native micro frontend delivery with Hot Updater compatibility, host-provided state, and package-manager neutral workflows.",
  ...
  ko: {
    suffix: `${brand} 한국어 문서`,
    overview: "Hot Updater 호환, Host shared state, native contract 검증을 제공하는 React Native micro frontend 한국어 가이드입니다.",
  ...
  "zh-CN": {
    suffix: `${brand} 简体中文文档`,
    overview: "面向 React Native micro frontend 的 native-safe delivery、Hot Updater 集成与 Host shared state 指南。",
  ...
  ja: {
    ...
    overview: "React Native micro frontend の native-safe delivery、Hot Updater 連携、Host shared state の日本語ガイドです。",
...
export function getAlternateLinks(urlOriginal: string) {
  ...
  return [
    ...
    { lang: "ko", href: absoluteSiteUrl(joinPath(localePrefixes.ko, routeWithoutLocale)) },
    { lang: "zh-CN", href: absoluteSiteUrl(joinPath(localePrefixes["zh-CN"], routeWithoutLocale)) },
    { lang: "ja", href: absoluteSiteUrl(joinPath(localePrefixes.ja, routeWithoutLocale)) },
  ];
}
  ...
  return JSON.stringify({
    ...
    "@graph": [
      ...
      },
      {
        ...
        publisher: { "@id": `${absoluteSiteUrl("/")}#organization` },
        about: ["React Native", "Micro Frontend", "OTA", "Hot Updater", "Native Contract"],
      },
      {
        "@type": "BreadcrumbList",
