...
const localeConfig = {
  ko: {
    ...
    homeSubtitle: "React Native feature module을 독립적으로 배포하고, native binary compatibility를 검증한 뒤 Host가 안전하게 로드할 수 있을 때만 Hot Updater로 publish합니다.",
    docsCta: "문서 보기",
    matrixCta: "패키지 매니저 표",
    docsEyebrow: "한국어 문서",
    hotTitle: "Hot Updater 설정",
    hotSubtitle: "기존 OTA engine을 바꾸지 않고 Hot Updater 앞단에 native-safety check를 추가하는 route입니다.",
  ...
  zh: {
    ...
    homeSubtitle: "独立发布 React Native feature modules，验证 native binary compatibility，并且只在 Host 可安全加载时通过 Hot Updater 发布。",
    docsCta: "查看文档",
    matrixCta: "包管理器矩阵",
    docsEyebrow: "简体中文文档",
    hotTitle: "Hot Updater 设置",
    hotSubtitle: "不替换现有 OTA engine，而是在 Hot Updater 前面加入 native-safety check。",
    packageTitle: "包管理器矩阵",
  ...
  jp: {
    ...
    basePath: "/jp",
    homeEyebrow: "React Native Micro Frontend",
    homeTitle: "native-safe な feature module を洗練された OTA workflow で公開します。",
    homeSubtitle: "React Native feature modules を独立して公開し、native binary compatibility を検証し、Host が安全にロードできる場合だけ Hot Updater で publish します。",
    docsCta: "ドキュメントを見る",
    matrixCta: "Package manager 一覧",
    docsEyebrow: "日本語ドキュメント",
    hotTitle: "Hot Updater 設定",
    hotSubtitle: "既存の OTA engine を置き換えず、Hot Updater の前段に native-safety check を追加する route です。",
...
export function LocalizedHomeRoute({ locale }: { readonly locale: LocaleCode }) {
  ...
  return (
    <ShellSection>
      <Hero
        ...
        secondaryCta={{ label: config.matrixCta, href: `${config.basePath}/docs/package-managers` }}
      />
...
export function LocalizedDocsRoute({ locale }: { readonly locale: LocaleCode }) {
  const config = localeConfig[locale];

  return (
    <ShellSection>
      <PageHeader title={config.guide.title} subtitle={config.guide.subtitle} eyebrow={config.docsEyebrow} />
      <LocalizedRouteCards basePath={config.basePath} cards={config.guide.cards} />
      <SectionList sections={config.guide.sections} />
    </ShellSection>
  );
}

export function LocalizedHotUpdaterRoute({ locale }: { readonly locale: LocaleCode }) {
  const config = localeConfig[locale];

  return (
    <ShellSection>
      <PageHeader title={config.hotTitle} subtitle={config.hotSubtitle} eyebrow={config.docsEyebrow} />
      <SectionList sections={config.guide.hotUpdaterSections} />
    </ShellSection>
  );
}

export function LocalizedPackageManagersRoute({ locale }: { readonly locale: LocaleCode }) {
  const config = localeConfig[locale];

  return (
    <ShellSection>
      <PageHeader title={config.packageTitle} subtitle={config.packageSubtitle} eyebrow={config.docsEyebrow} />
      <MatrixTable rows={installMatrix} headers={config.tableHeaders} />
    </ShellSection>
  );
}

export function LocalizedNativeContractRoute({ locale }: { readonly locale: LocaleCode }) {
  const config = localeConfig[locale];

  return (
    <ShellSection>
      <PageHeader title={config.nativeTitle} subtitle={config.nativeSubtitle} eyebrow={config.docsEyebrow} />
