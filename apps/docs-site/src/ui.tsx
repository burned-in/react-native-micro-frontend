      ...
      >
        ...
        <div className={wrap({ gap: '3' })}>
          <a
            ...
            href={withSiteBase(primaryCta.href)}
          >
            {primaryCta.label}
          </a>
          <a
            className={button({ tone: 'ghost' })}
            href={withSiteBase(secondaryCta.href)}
          >
            {secondaryCta.label}
          </a>
        </div>
...
export function DocBlock({ section }: { readonly section: DocSection }) {
  return (
    <section className={cx(card(), css({ p: { base: '6', md: '8' } }))}>
      <div
        className={grid({
          columns: { base: 1, lg: section.code ? 2 : 1 },
          gap: '8',
          alignItems: 'start',
        })}
      >
...
const button = (props: { readonly tone: 'primary' | 'ghost' }) =>
  css({
    display: 'inline-flex',
    alignItems: 'center',
...
const tableCell = (props?: { readonly strong?: boolean }) =>
  css({
    p: '4',
    borderBottomWidth: '1px',
    borderColor: 'line',
