...
export const guideCards: readonly Feature[] = [
  {
    title: 'Getting started',
    body: 'Copy, paste, and run the first safe Host + MFE setup from install to runtime loading.',
  },
  {
    title: 'Easy Way',
    body: 'Pick one path first: Bundle archive, Hot Updater/custom OTA, or Expo EAS Update.',
  },
  {
    title: 'Hot Updater setup',
    body: 'Keep Hot Updater as delivery while RNM blocks native-unsafe OTA updates before publish.',
  },
  {
    title: 'Metro / Bundle archive',
    body: 'Use Metro-first archives with manifest.json, index.bundle, and only referenced assets.',
  },
  {
    title: 'Package managers & CLI',
    body: 'Use the same RNM commands with Bun, npm, pnpm, Yarn, or Deno.',
  },
  {
    title: 'Options reference',
    body: 'Find which file owns each Host config, MFE config, registry, and runtime option.',
  },
  {
    title: 'Native contract',
    body: 'Understand which changes are OTA-safe and which require a store release.',
  },
  {
    title: 'RNM vs Re.Pack',
    body: 'Compare Metro-first native-safety with Re.Pack Module Federation architecture.',
  },
  {
    title: 'Global state',
    body: 'Pass host-owned session, locale, feature flags, and tenant context into MFEs safely.',
  },
];
...
export const gettingStartedSections: readonly DocSection[] = [
  {
    ...
    title: 'Build one small feature module first.',
    body: [
      'Follow this page like a recipe. Create or open two folders: host-app is the installed React Native app, and mfe-feature is one independently shipped feature.',
      'RNM does not replace Hot Updater. RNM checks whether the feature is safe for the current native binary; Hot Updater, Expo, bundle archives, or your own loader still delivers the JavaScript.',
    ],
    code: `workspace/
  host-app/      # owns navigation, native binary, registry, fallback UI
  mfe-feature/   # owns one feature, default-exported React component`,
  },
  {
    eyebrow: 'Step 1',
    title: 'Install RNM in the Host App.',
    body: [
      'Run these commands inside host-app. Bun is shown first because this repository is Bun-first, but the package manager page has npm, pnpm, Yarn, and Deno equivalents.',
      'init writes reviewable configuration files. It should not feel like magic native patching.',
    ],
    code: `cd host-app
...
bunx @bunin/react-native-micro-frontend-cli init`,
  ...
  {
    eyebrow: 'Step 2',
    title: 'Keep the first Host policy simple.',
    body: [
      'Start with manual OTA and manual native integration. This makes every generated change visible while you learn the workflow.',
      'Leave mfes empty. rnm add will write runtime module registration to rnm.registry.json later.',
    ],
...
export default defineReactNativeMicroFrontendConfig({
  ...
  packageManager: {
    supported: ["bun", "npm", "pnpm", "yarn", "deno"],
  ...
  {
    eyebrow: 'Step 3',
    title: 'Create the smallest MFE entry file.',
    body: [
      'The MFE entry must default-export a React component. Keep the first one boring so you can prove the pipeline before adding real product code.',
      'The registry entry will point to this file, Metro will bundle it, and the Host loader will read module.default.',
    ],
...
export default function MfeFeature() {
  return (
    <View style={{ padding: 16 }}>
      <Text>Hello from mfe-feature</Text>
    </View>
  );
}`,
  },
  {
    eyebrow: 'Step 4',
    title: 'Register the MFE, then verify before publishing.',
    body: [
      'Run these commands in host-app. rnm add creates or updates rnm.registry.json; rnm verify checks native compatibility; rnm publish should happen only after verification passes.',
      'If verification fails, do not force OTA. Ship a native store release or remove the native-changing dependency first.',
    ],
    code: `cd host-app
...
rnm publish mfe-feature --package-manager bun --channel production`,
  ...
  {
    eyebrow: 'Step 5',
    title: 'Mount it with a Host-owned loader.',
    body: [
      'The runtime checks missing modules, blocked modules, and native hash mismatches. Your Host still owns the real JavaScript loading implementation and fallback UI.',
      'Use Hot Updater for OTA, createBundleArchiveLoader for copied archives, or a custom callback for your own CDN/runtime.',
    ],
...
export function App() {
  return (
    <MicroFrontendProvider registry={registryJson as MfeRegistry}>
      <MicroFrontendComponent
        ...
        errorFallback={(error) => <Loading reason={String(error)} />}
      />
    </MicroFrontendProvider>
  );
}`,
  ...
  {
    eyebrow: 'Step 6',
    title: 'Add Metro wiring only when you need local development or archives.',
    body: [
      'withMfe reads rnm.registry.json, adds active MFE roots to watchFolders, and maps shared packages to the Host node_modules.',
      'Use this when you develop the Host and MFE together or when you create Metro-first bundle archives.',
    ],
    code: `// host-app/metro.config.js
const { getDefaultConfig, mergeConfig } = require("@react-native/metro-config");
...
module.exports = (async () => {
  ...
  {
    eyebrow: 'Next choice',
    title: 'Choose one delivery path next.',
    body: [
      'If you want remote OTA, continue to Hot Updater setup. If you want a portable local artifact, continue to Metro / Bundle archive. If the Host is Expo, use the Expo route in Easy Way.',
      'This is the same mental model as Hot Updater: a short install path first, then focused docs for the delivery mechanism you actually need.',
    ],
    code: `# Remote OTA with Hot Updater
rnm verify mfe-feature
rnm publish mfe-feature --package-manager bun --channel production

# Portable archive instead
cd ../mfe-feature
rnm bundle mfe-feature --platform ios --host ../host-app --yes --update-registry
