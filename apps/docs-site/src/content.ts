...
rnm bundle mfe-feature --platform ios --host ../host-app --yes`,
  },
  {
    ...
    body: [
      ...
      'rnm init and rnm add pre-generate rnm.bundle-archives.ts and auto-import it from the Host entry. Later, rnm bundle --host regenerates the same file when the archive list changes. When the Host package has react-native-blob-util, the generated file registers createReactNativeBlobUtilAssetFileSystem automatically. Then createBundleArchiveLoader({ runtime: "react-native" }) is enough; pass assetFileSystem manually only for react-native-fs, Expo FileSystem, or custom native modules.',
...
// Generated by rnm bundle --host when react-native-blob-util is installed
...
function resolveOptionalReactNativeBlobUtil(): unknown {
  const nativeModules = ReactNative.NativeModules as
    | Record<string, unknown>
    | undefined;
  if (!nativeModules?.ReactNativeBlobUtil && !nativeModules?.RNFetchBlob) {
    return undefined;
  }
  try {
    ...
    metroBundleSections: [
      ...
      },
      {
        ...
        body: [
          ...
          'rnm init과 rnm add는 Host에 필요한 rnm.bundle-archives.ts를 미리 생성하고 Host entry에 자동 import합니다. 이후 rnm bundle --host는 archive 목록이 바뀔 때 같은 파일을 다시 생성합니다. Host package에 react-native-blob-util이 있으면 generated file에 createReactNativeBlobUtilAssetFileSystem도 자동 등록됩니다. 이 경우 createBundleArchiveLoader({ runtime: "react-native" })만으로 충분하며, react-native-fs/Expo/custom native module을 쓸 때만 assetFileSystem을 직접 넘기면 됩니다.',
          ...
          'cache path는 <cacheRoot>/rnm-assets/<mfeName>/<version>/<platform>/<bundleHash>/이며 .rnm-assets-ready.json marker가 있을 때만 재사용합니다. 생성된 registration은 react-native/Libraries/Image/AssetRegistry도 externalize하므로 Metro numeric asset ID가 extracted file:// URI로 resolve됩니다. minitax 같은 Host는 RNM package update/link, react-native-blob-util 유지, init/add의 generated registration 자동 import 확인, rnm bundle --host --yes 재실행만 확인하면 됩니다.',
        ],
    ...
    metroBundleSections: [
      ...
      },
      {
        ...
        body: [
          ...
          'rnm init 和 rnm add 会预先生成 rnm.bundle-archives.ts 并自动从 Host entry 导入；之后 rnm bundle --host 会在 archive 列表变化时重新生成同一文件。当 Host package 有 react-native-blob-util 时，generated file 会自动注册 createReactNativeBlobUtilAssetFileSystem。此时 createBundleArchiveLoader({ runtime: "react-native" }) 就足够；只有使用 react-native-fs、Expo 或 custom native module 时才需要手动传 assetFileSystem。',
          ...
          'cache path 为 <cacheRoot>/rnm-assets/<mfeName>/<version>/<platform>/<bundleHash>/，只有 .rnm-assets-ready.json marker 存在时才复用。生成的 registration 也会 externalize react-native/Libraries/Image/AssetRegistry，因此 Metro numeric asset ID 会 resolve 到 extracted file:// URI。minitax 类 Host 只需更新/link RNM package、保留 react-native-blob-util、确认 init/add 已自动 import generated registration，并重新运行 rnm bundle --host --yes。',
        ],
        code: metroBundleSections[3]?.code ?? '',
      },
    ],
    ...
    metroBundleSections: [
      ...
      },
      {
        ...
        body: [
          ...
          'rnm init と rnm add は rnm.bundle-archives.ts を事前生成し、Host entry から自動 import します。その後 rnm bundle --host は archive list が変わったときに同じ file を再生成します。Host package に react-native-blob-util がある場合、generated file は createReactNativeBlobUtilAssetFileSystem を自動登録します。この場合 createBundleArchiveLoader({ runtime: "react-native" }) だけで十分で、react-native-fs/Expo/custom native module を使う場合だけ assetFileSystem を手動で渡します。',
          ...
          'cache path は <cacheRoot>/rnm-assets/<mfeName>/<version>/<platform>/<bundleHash>/ で、.rnm-assets-ready.json marker がある場合だけ再利用します。生成 registration は react-native/Libraries/Image/AssetRegistry も externalize するため、Metro numeric asset ID は extracted file:// URI に resolve されます。minitax のような Host では RNM package update/link、react-native-blob-util 維持、init/add の generated registration 自動 import 確認、rnm bundle --host --yes 再実行だけ確認してください。',
        ],
        code: metroBundleSections[3]?.code ?? '',
      },
    ],
