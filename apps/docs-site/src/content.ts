...
export const hotUpdaterSections: readonly DocSection[] = [
  {
    ...
    title: "Keep Hot Updater as delivery, add native-safety verification before it.",
    body: [
...
export const localizedGuides = {
  ko: {
    ...
    subtitle: "설치, Host 설정, MFE 등록, runtime 로딩, Host-provided shared state, Hot Updater 배포까지 같은 정보 구조로 정리한 한국어 문서입니다.",
    cards: [
    ...
    hotUpdaterSections: [
      {
        eyebrow: "Hot Updater route",
        title: "Hot Updater는 delivery로 유지하고, 앞단에 native-safety verification을 추가합니다.",
        body: [
      ...
      },
      {
        ...
        body: ["Host team이 서로 다른 package manager를 사용해도 safety rule은 같습니다. 달라지는 것은 command runner뿐입니다."],
    ...
    nativeContractSections: [
      ...
      },
      {
        ...
        body: ["JS-only 변경은 OTA 후보가 될 수 있습니다. native ABI, dependency, runtime flag, host/module native-hash mismatch는 store release가 필요합니다."],
  ...
  zh: {
    ...
    subtitle: "这里用相同的信息结构说明安装、Host 配置、MFE 注册、runtime 加载、Host-provided shared state 与 Hot Updater 发布。",
    cards: [
    ...
    sections: [
      ...
      },
      {
        ...
        body: ["CLI 生成可审查的 integration file，注册 feature module，比较 native contract，并且只在 OTA safety check 通过后输出 Hot Updater deploy command。"],
        code: (docsSections[1]!.code ?? ""),
      },
      {
        eyebrow: "Runtime",
        title: "通过明确的 runtime policy 加载 feature modules。",
        body: ["Runtime 读取 registry，并拒绝 blocked module、missing module 和 native-hash mismatch。fallback UI 由 Host 负责。"],
        code: (docsSections[2]!.code ?? ""),
      },
      {
        eyebrow: "全局状态",
    ...
    hotUpdaterSections: [
      {
        eyebrow: "Hot Updater route",
        title: "保留 Hot Updater 作为 delivery，并在前面添加 native-safety verification。",
        body: [
      ...
      },
      {
        eyebrow: "Publish",
        title: "先校验，再发布。",
        body: ["即使 Host team 使用不同的 package manager，safety rule 也相同；变化的只有 command runner。"],
        code: (hotUpdaterSections[2]!.code ?? ""),
      },
    ],
    ...
    nativeContractSections: [
      ...
      },
      {
        ...
        body: ["JS-only 变更可以成为 OTA 候选。native ABI、dependency、runtime flag、host/module native-hash mismatch 都需要 store release。"],
        code: (nativeContractSections[1]!.code ?? ""),
      },
    ],
  },
  ja: {
    ...
    subtitle: "install、Host 設定、MFE 登録、runtime loading、Host-provided shared state、Hot Updater publish まで同じ情報構造で整理した日本語ドキュメントです。",
    cards: [
    ...
    sections: [
      ...
      },
      {
        ...
        body: ["CLI は review 可能な integration file を生成し、feature module を登録し、native contract を比較し、OTA safety check が通った場合だけ Hot Updater deploy command を出力します。"],
        code: (docsSections[1]!.code ?? ""),
      },
      {
        eyebrow: "Runtime",
        title: "明示的な runtime policy で feature modules をロードします。",
        body: ["Runtime は registry を読み、blocked module、missing module、native-hash mismatch を拒否します。fallback UI は Host が所有します。"],
        code: (docsSections[2]!.code ?? ""),
      },
      {
        eyebrow: "Global state",
    ...
    hotUpdaterSections: [
      {
        eyebrow: "Hot Updater route",
        title: "Hot Updater を delivery として維持し、その前段に native-safety verification を追加します。",
        body: [
      ...
      {
        eyebrow: "Adapter",
        title: "既存の Hot Updater config を MFE metadata と一緒に wrap します。",
        body: [
      ...
      },
      {
        ...
        body: ["Host team が異なる package manager を使っても safety rule は同じです。変わるのは command runner だけです。"],
        code: (hotUpdaterSections[2]!.code ?? ""),
      },
    ],
    ...
    nativeContractSections: [
      {
        eyebrow: "Native contract",
        title: "native hash は JavaScript が変更できない前提を capture します。",
        body: [
      ...
      {
        eyebrow: "Decision rule",
        title: "OTA は optimization であり、native release の抜け道ではありません。",
        body: ["JS-only change は OTA の候補になります。native ABI、dependency、runtime flag、host/module native-hash mismatch は store release が必要です。"],
        code: (nativeContractSections[1]!.code ?? ""),
      },
    ],
  },
