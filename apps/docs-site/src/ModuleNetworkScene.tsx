...
export function ModuleNetworkScene() {
  ...
  useEffect(() => {
    ...
    async function mountScene() {
      ...
      const violetLine = new THREE.LineBasicMaterial({
        color: 0xc084fc,
        transparent: true,
        opacity: 0.58,
      });
      const _softWhiteLine = new THREE.LineBasicMaterial({
        color: 0xf0f9ff,
        transparent: true,
        opacity: 0.74,
      });
      const cyanSprite = new THREE.MeshBasicMaterial({
        color: 0x67e8f9,
        transparent: true,
        opacity: 0.9,
      });
      const violetSprite = new THREE.MeshBasicMaterial({
        color: 0xc084fc,
        transparent: true,
        opacity: 0.86,
      });
      const emeraldSprite = new THREE.MeshBasicMaterial({
        color: 0x34d399,
        transparent: true,
        opacity: 0.8,
      });
      const whiteSprite = new THREE.MeshBasicMaterial({
        color: 0xf8fafc,
        transparent: true,
        opacity: 0.82,
      });
      ...
      moduleData.forEach((data, index) => {
        ...
        const material = (
          cardPalette[index % cardPalette.length] ?? cyanSprite
        ).clone();
        const card = new THREE.Mesh(cardGeometry, material);
        const edge = new THREE.LineSegments(
          cardEdgeGeometry,
          index % 2 === 0 ? cyanLine : violetLine,
        );
        const dot = new THREE.Mesh(
          cardDotGeometry,
          index % 3 === 0 ? emeraldSprite : whiteSprite,
        );
        const firstBar = new THREE.Mesh(cardBarGeometry, deepGlass);
        const secondBar = new THREE.Mesh(
          cardBarGeometry,
          index % 2 === 0 ? violetSprite : emeraldSprite,
        );
      ...
      moduleData.forEach((data, index) => {
        ...
        const line = new THREE.Mesh(
          geometry,
          new THREE.MeshBasicMaterial({
            color: index % 2 === 0 ? 0x67e8f9 : 0xc084fc,
            transparent: true,
            opacity: 0.44,
          }),
        );
      ...
      const orbitRings = [
        ...
        new THREE.Mesh(
          new THREE.TorusGeometry(3.08, 0.007, 8, 220),
          violetSprite,
        ),
        new THREE.Mesh(
          new THREE.TorusGeometry(1.92, 0.006, 8, 220),
          emeraldSprite,
        ),
      ];
      ...
      const render = (time: number) => {
        ...
        root.rotation.y =
          -0.42 +
          smoothRotationY +
          (reducedMotion ? 0 : Math.sin(seconds * 0.2) * 0.14);
        root.rotation.x =
          -0.12 +
          smoothRotationX +
          (reducedMotion ? 0 : Math.sin(seconds * 0.32) * 0.05);
        ...
        if (!reducedMotion) {
          ...
          moduleCards.forEach(
            ({ group, baseX, baseY, baseZ, phase }, index) => {
              group.position.x =
                baseX + Math.sin(seconds * 0.72 + phase) * 0.035;
              group.position.y =
                baseY + Math.sin(seconds * 1.04 + phase) * 0.08;
              group.position.z =
                baseZ + Math.cos(seconds * 0.82 + phase) * 0.055;
              group.rotation.z +=
                Math.sin(seconds * 0.78 + phase + index) * 0.0009;
            },
          );

          orbitNodes.forEach(
            ({ mesh, radius, angle, speed, height }, index) => {
              const current = angle + seconds * speed;
              mesh.position.x = Math.cos(current) * radius;
              mesh.position.z = Math.sin(current) * 0.88;
              mesh.position.y =
                Math.sin(current * 1.36 + index * 0.35) * height;
            },
          );
        }
  ...
  return (
    <div
      ...
      className={css({
        ...
        zIndex: '1',
        inset: {
          base: 'auto -118px -92px auto',
          sm: 'auto -90px -112px auto',
          lg: '-4px -2px 4px auto',
        },
        ...
        pointerEvents: 'auto',
        filter:
          'drop-shadow(0 38px 110px rgba(103, 232, 249, 0.42)) drop-shadow(0 18px 54px rgba(192, 132, 252, 0.34))',
