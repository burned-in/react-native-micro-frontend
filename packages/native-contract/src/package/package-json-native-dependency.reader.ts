 ...
 */
export function readNativePackageDependencies(packageJson: {
  readonly dependencies?: Record<string, string>;
  readonly peerDependencies?: Record<string, string>;
}): readonly NativePackageDependency[] {
  const deps = {
    ...(packageJson.dependencies ?? {}),
    ...(packageJson.peerDependencies ?? {}),
  };
  return Object.entries(deps)
    .map(([name, version]) => classifyDependency(name, version))
...
export function classifyDependency(
  ...
  if (KNOWN_JS_ONLY.has(name))
    return { name, version, native: false, reason: 'known JS-only dependency' };
  if (name === 'react-native')
    return {
      name,
      version,
      native: true,
      reason: 'React Native runtime version is native-contract critical',
    };
  if (
    ...
    return {
      ...
      version,
      native: true,
      reason: 'React Native ecosystem package may include native code',
    };
  }
