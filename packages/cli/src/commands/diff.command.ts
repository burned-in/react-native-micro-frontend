...
export function runDiffCommand(
  ...
  if (!existsSync(registryPath) || !existsSync(hostContractPath)) {
    printer.error(
      'Missing rnm.registry.json or rnm.native-contract.json. Run `rnm init` first.',
    );
    return 1;
  }
  ...
  const nativeBinaryChangeRequired = changes.some(
    (change) => change.nativeBinaryChange,
  );
  ...
  printer.log(
    `Native binary change required: ${nativeBinaryChangeRequired ? 'YES' : 'NO'}`,
  );
  printer.log(`OTA possible: ${nativeBinaryChangeRequired ? 'NO' : 'YES'}`);
  printer.log(
    `Store release required: ${nativeBinaryChangeRequired ? 'YES' : 'NO'}`,
  );
