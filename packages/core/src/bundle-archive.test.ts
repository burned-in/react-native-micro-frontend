...
describe('bundle archive loader', () => {
  ...
  test('does not enumerate React Native namespace getters while patching archive assets', async () => {
    const archive = createFixtureArchive({
      name: manifest.name,
      entryModuleId: 0,
      moduleGlobalName: '__rnm_mfe_module__',
      ...
      metroModuleId: {
        'react-native': 1,
        'react-native/Libraries/Image/AssetRegistry': 2,
      },
      externalModules: [
        'react-native',
        'react-native/Libraries/Image/AssetRegistry',
      ...
      bundleCode: `
        ...
        __d(function () { throw new Error('bundled react-native initialized'); }, 1, []);
        __d(function () { throw new Error('bundled AssetRegistry initialized'); }, 2, []);
        __d(function (global, require, importDefault, importAll, module, exports, dependencyMap) {
          const ReactNative = require(dependencyMap[0]);
          const AssetRegistry = require(dependencyMap[1]);
          const assetId = AssetRegistry.registerAsset({
          ...
          module.exports.default = function LazyReactNativeGetterMfe() {
            return ReactNative.Image.resolveAssetSource(assetId).uri;
          };
        }, 0, [1, 2]);
      `,
    });
    let imageGetterCalls = 0;
    let keyboardGetterCalls = 0;
    const reactNative = {};
    Object.defineProperties(reactNative, {
      Image: {
        ...
        get() {
          imageGetterCalls += 1;
          return { resolveAssetSource: () => ({ uri: 'host://fallback' }) };
        },
      },
      Keyboard: {
        enumerable: true,
        get() {
          keyboardGetterCalls += 1;
          throw new Error(
    ...
    expect(module.default()).toBe('data:image/jpeg;base64,anBn');
    expect(imageGetterCalls).toBe(0);
    expect(keyboardGetterCalls).toBe(0);
  });
