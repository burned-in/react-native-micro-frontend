import type {
  CheckOtaEligibilityInput,
  OtaBlockReason,
  OtaEligibilityResult,
  OtaEligibilityStatus,
} from './ota-eligibility.type.js';

function reason(
  code: string,
  message: string,
  storeReleaseRequired: boolean,
): OtaBlockReason {
 ...
 */
export function checkOtaEligibility(
  input: CheckOtaEligibilityInput,
): OtaEligibilityResult {
  if (!input.otaEnabled) {
    return blocked(
      'disabled',
      [reason('OTA_DISABLED', 'OTA is disabled for this MFE.', false)],
      false,
      false,
    );
  }
  ...
  if (input.reactNativeVersionMismatch) {
    return blocked(
      ...
      [
        reason(
          'RN_VERSION_CHANGED',
          'React Native version changed. Full app release is required.',
          true,
        ),
      ],
      true,
  ...
  if (input.hermesSettingMismatch) {
    return blocked(
      ...
      [
        reason(
          'HERMES_SETTING_CHANGED',
          'Hermes setting changed. Store release is required.',
          true,
        ),
      ],
      true,
  ...
  if (input.newArchitectureSettingMismatch) {
    return blocked(
      ...
      [
        reason(
          'NEW_ARCHITECTURE_SETTING_CHANGED',
          'New Architecture setting changed. Store release is required.',
          true,
        ),
      ],
      true,
  ...
  if (input.nativeChanged) {
    ...
    return blocked(
      ...
      nativeReasons.length > 0
        ? nativeReasons
        : [reason('NATIVE_CHANGE', 'Native binary change detected.', true)],
      true,
      true,
    );
  }
    ...
    return blocked(
      ...
      [
        reason(
          ...
          'nativeHash does not match the host binary.',
          true,
        ),
      ],
      true,
  ...
  if (input.mfeBlocked) {
    return blocked(
      ...
      [
        reason(
          'MFE_BLOCKED',
          'MFE is blocked because required native changes were not applied.',
          false,
        ),
      ],
      false,
  ...
  if (input.policyReason) {
    return blocked(
      'blocked-by-policy',
      [reason('POLICY_BLOCK', input.policyReason, false)],
      false,
      false,
    );
  }
  ...
  return {
    status: 'available',
    otaPossible: true,
    storeReleaseRequired: false,
    runtimeLoadBlocked: false,
    reasons: [],
  };
}

function blocked(
  status: OtaEligibilityStatus,
  reasons: readonly OtaBlockReason[],
  storeReleaseRequired: boolean,
  runtimeLoadBlocked: boolean,
): OtaEligibilityResult {
  return {
    status,
    otaPossible: false,
