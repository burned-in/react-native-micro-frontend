...
export function detectPackageManager(input: {
  ...
  const choose = (
    ...
    reason,
    evidence,
    crossProjectWarning: Boolean(
      input.otherProjectPackageManager &&
        input.otherProjectPackageManager !== name,
    ),
  });
 ...
 */
export function createPackageManagerCommands(
    ...
    case 'deno':
      return {
        ...
        installDependencies: (deps) => [
          'echo',
          `Deno projects require manual dependency setup: ${deps.join(' ')}`,
        ],
      };
    case 'pnpm':
      return {
