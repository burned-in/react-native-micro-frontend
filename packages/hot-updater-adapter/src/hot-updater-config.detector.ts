...
export function detectHotUpdater(root: string): HotUpdaterDetection {
  ...
  const base = {
    detected: Boolean(
      configPath || deps['@hot-updater/react-native'] || deps['hot-updater'],
    ),
    packageDetected: Boolean(
      deps['@hot-updater/react-native'] || deps['hot-updater'],
    ),
    wrapCallDetected: /HotUpdater\.wrap\(/.test(configText),
    envDetected: existsSync(join(root, '.env.hotupdater')),
    deployScriptDetected: Object.values(packageJson.scripts ?? {}).some(
      (script) => script.includes('hot-updater') && script.includes('deploy'),
    ),
  };
