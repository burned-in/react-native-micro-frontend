...
export function getRuntimeMfeState(registry: MfeRegistry, name: string): RuntimeMfeState {
  ...
  if (!manifest) return { name, status: "missing", reason: "MFE is not registered." };
  if (manifest.status === "blocked") return { name, status: "blocked", manifest, reason: manifest.blockedReason ?? "MFE is blocked." };
  if (registry.hostNativeHash && manifest.nativeHash && registry.hostNativeHash !== manifest.nativeHash) return { name, status: "blocked", manifest, reason: "nativeHash mismatch." };
